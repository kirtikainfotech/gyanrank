...
$whatsappText = rawurlencode('Hello Gyan Nexa support, I need help with my account.');
$supportMarkup = <<<HTML

<style>
.gyannexa-whatsapp-support{position:fixed;right:22px;bottom:22px;z-index:9999;width:54px;height:54px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:#25d366;color:#fff;box-shadow:0 14px 34px rgba(7,94,84,.24);border:3px solid #fff;transition:transform .18s ease,box-shadow .18s ease}
.gyannexa-whatsapp-support:hover{transform:translateY(-2px);box-shadow:0 18px 42px rgba(7,94,84,.32)}
.gyannexa-whatsapp-support svg{width:29px;height:29px;fill:currentColor}
.gyannexa-login-helper{display:flex;justify-content:flex-end;margin:-6px 0 14px;font:600 14px Arial,sans-serif}
.gyannexa-login-helper a{color:#004074;text-decoration:none}
.gyannexa-login-helper a:hover{color:#f68a00}
@media(max-width:640px){.gyannexa-whatsapp-support{right:16px;bottom:78px;width:50px;height:50px}}
</style>
<a class="gyannexa-whatsapp-support" href="https://wa.me/{$whatsappNumber}?text={$whatsappText}" target="_blank" rel="noopener" aria-label="Chat with Gyan Nexa support on WhatsApp">
